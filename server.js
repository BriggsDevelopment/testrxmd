...
env.addFilter(
  "formatDateTime",
  (date) => new Date(date).toISOString().slice(0, -1).split(".")[0]
);
        ...
        (error, accounts) => {
          ...
          if (accounts.length > 0) {
            ...
            if (
              ...
              accounts[0].activation_code != "activated" &&
              accounts[0].activation_code != ""
            ) {
            ...
            // do not change the message as the ajax code depends on it
            return response.end();
          } else {
...
app.get("/register", (request, response) =>
  isLoggedin(
    ...
    (settings) => {
      ...
      request.session.token = token;
      // User is not logged in, render login template
      response.render("login.html", { token: token, settings: settings });
    }
  )
);
  ...
  init(request, (settings) => {
    ...
    if (username && password && email) {
      ...
      connection.query(
        ...
        [username, email],
        (error, accounts, fields) => {
          // Check if account exists and validate input data
          if (accounts.length > 0) {
            response.send(
              "Account already exists with that username and/or email!"
            );
            response.end();
            ...
            let activationTemplate = fs
              .readFileSync(
                path.join(__dirname, "views/activation-email-template.html"),
                "utf8"
              )
              .replaceAll("%link%", activateLink);
            ...
            connection.query(
              "INSERT INTO accounts (username, password, email, activation_code, role, ip) VALUES (?, ?, ?, ?, ?, ?)",
              [username, hashedPassword, email, activationCode, role, ip],
              () => {
            ...
            // Insert account
            connection.query(
              'INSERT INTO accounts (username, password, email, activation_code, role, ip) VALUES (?, ?, ?, "activated", ?, ?)',
              [username, hashedPassword, email, role, ip],
              (error, result) => {
  ...
  connection.query(
    ...
    (error, accounts) => {
      ...
      if (accounts.length > 0) {
        ...
        response.render("activate.html", {
          msg: "Incorrect email and/or activation code!",
        });
      }
    }
  );
      ...
      connection.query(
        ...
        (error, accounts) => {
          ...
          if (accounts.length > 0) {
            ...
            connection.query("UPDATE accounts SET reset = ? WHERE email = ?", [
              resetCode,
              request.body.email,
            ]);
  ...
  if (request.params.email && request.params.code) {
    ...
    connection.query(
      ...
      (error, accounts) => {
        ...
        if (accounts.length > 0) {
          // Render forgot password template
          response.render("resetpassword.html", {
        ...
        if (accounts.length > 0) {
          ...
          if (request.body.npassword && request.body.cpassword) {
            ...
            if (request.body.npassword != request.body.cpassword) {
              ...
              request.body.npassword.length < 5 ||
              request.body.npassword.length > 20
            ) {
              msg = "Password must be between 5 and 20 characters long!";
            } else {
              // Success! Update password
              msg =
                'Your password has been reset! You can now <a href="/">login</a>!';
              // Hash password
              let hashedPassword = cryptography
                .createHash("sha1")
                .update(request.body.npassword)
                .digest("hex");
              // Update password
              connection.query(
                'UPDATE accounts SET password = ?, reset = "" WHERE email = ?',
                [hashedPassword, request.params.email]
              );
            }
  ...
  isLoggedin(
    ...
    (settings) => {
      ...
      connection.query(
        ...
        [request.session.account_username],
        (error, accounts, fields) => {
          // Format the registered date
          accounts[0].registered = new Date(accounts[0].registered)
            .toISOString()
            .split("T")[0];
          // Render profile page
      ...
      connection.query(
        ...
        (error, accounts, fields) => {
          ...
          hashedPassword = !password
            ? request.session.account_password
            : hashedPassword;
          ...
          if (
            ...
            connection.query(
              "UPDATE accounts SET username = ?, password = ?, email = ?, activation_code = ? WHERE username = ?",
              [
                username,
                hashedPassword,
                email,
              ...
              () => {
                ...
                if (requiresActivation) {
                  ...
                  // Update msg
                  msg =
                    "You have changed your email address! You need to re-activate your account! You will be automatically logged-out.";
...
app.get("/admin/", (request, response) =>
  isAdmin(
    ...
    (settings) => {
      ...
      connection.query(
        "SELECT * FROM accounts WHERE cast(registered as DATE) = cast(now() as DATE) ORDER BY registered DESC; SELECT COUNT(*) AS total FROM accounts LIMIT 1; SELECT COUNT(*) AS total FROM accounts WHERE last_seen < date_sub(now(), interval 1 month) LIMIT 1; SELECT * FROM accounts WHERE last_seen > date_sub(now(), interval 1 day) ORDER BY last_seen DESC; SELECT COUNT(*) AS total FROM accounts WHERE last_seen > date_sub(now(), interval 1 month) LIMIT 1",
...
app.get(
  [
    "/admin/accounts",
    "/admin/accounts/:msg/:search/:status/:activation/:role/:order/:order_by/:page",
  ],
  (request, response) =>
    isAdmin(
      request,
      (settings) => {
        ...
        let order_by = order_by_whitelist.includes(request.params.order_by)
        ...
        if (status == "active") {
          where += where
            ? "AND last_seen > date_sub(now(), interval 1 month) "
          ...
          (error, results) => {
            ...
            connection.query(
              ...
              (error, accounts) => {
                ...
                response.render("admin/accounts.html", {
                  selected: "accounts",
                  selectedChild: "view",
                  accounts: accounts,
...
app.get(["/admin/account", "/admin/account/:id"], (request, response) =>
  isAdmin(
    request,
    (settings) => {
      ...
      let account = {
        ...
        rememberme: "",
        role: "Member",
        registered: new Date(d.getTime() - d.getTimezoneOffset() * 60000)
          .toISOString()
          .slice(0, -1)
          .split(".")[0],
        last_seen: new Date(d.getTime() - d.getTimezoneOffset() * 60000)
          .toISOString()
          .slice(0, -1)
          .split(".")[0],
      };
      ...
      if (request.params.id) {
        connection.query(
          ...
          (error, accounts) => {
            account = accounts[0];
            response.render("admin/account.html", {
              selected: "accounts",
              selectedChild: "manage",
              page: page,
              roles: roles,
              account: account,
            });
...
// http://localhost:3000/admin/account - Admin edit/create account
app.post(["/admin/account", "/admin/account/:id"], (request, response) =>
  isAdmin(
    request,
    (settings) => {
      // GET request ID exists, edit account
...
app.get("/admin/account/delete/:id", (request, response) =>
  isAdmin(
    request,
    (settings) => {
      // GET request ID exists, delete account
      if (request.params.id) {
        connection.query("DELETE FROM accounts WHERE id = ?", [
          request.params.id,
        ]);
...
app.get("/admin/roles", (request, response) =>
  isAdmin(
    request,
    (settings) => {
      // Roles list
      ...
      connection.query(
        "SELECT role, COUNT(*) as total FROM accounts GROUP BY role; SELECT role, COUNT(*) as total FROM accounts WHERE last_seen > date_sub(now(), interval 1 month) GROUP BY role; SELECT role, COUNT(*) as total FROM accounts WHERE last_seen < date_sub(now(), interval 1 month) GROUP BY role",
        (error, roles) => {
          // Roles array
          new_roles = {};
          ...
          for (const role in roles[2]) {
            new_roles_inactive[roles[2][role]["role"]] =
              roles[2][role]["total"];
          }
          // Render roles template
          response.render("admin/roles.html", {
            selected: "roles",
            roles: new_roles,
            roles_active: new_roles_active,
            roles_inactive: new_roles_inactive,
          });
        }
      );
    },
...
app.get(
  ["/admin/emailtemplate", "/admin/emailtemplate/:msg"],
  (request, response) =>
    isAdmin(
      request,
      (settings) => {
        ...
        // Read template files
        const activation_email_template = fs.readFileSync(
          path.join(__dirname, "views/activation-email-template.html"),
          "utf8"
        );
        const twofactor_email_template = fs.readFileSync(
        ...
        response.render("admin/emailtemplates.html", {
          selected: "emailtemplate",
          msg: msg,
          activation_email_template: activation_email_template,
          twofactor_email_template: twofactor_email_template,
        });
      },
      () => {
        // Redirect to login page
  ...
  (request, response) =>
    isAdmin(
      ...
      (settings) => {
        // If form submitted
        if (
          request.body.activation_email_template &&
          request.body.twofactor_email_template
        ) {
...
app.get(["/admin/settings", "/admin/settings/:msg"], (request, response) =>
  isAdmin(
    request,
    (settings) => {
      // Output message
      ...
      getSettings((settings) => {
        ...
        response.render("admin/settings.html", {
          selected: "settings",
          msg: msg,
          settings: settings,
          settingsFormatTabs: settingsFormatTabs,
          settingsFormatForm: settingsFormatForm,
        });
      });
    },
    () => {
...
app.post(["/admin/settings", "/admin/settings/:msg"], (request, response) =>
  isAdmin(
    request,
    (settings) => {
      // Update settings
...
app.get("/admin/myaccount", (request, response) =>
  isAdmin(
    request,
    (settings) => {
...
app.get("/admin/about", (request, response) =>
  isAdmin(
    request,
    (settings) => {
...
const init = (request, callback) => {
  if (request.session.account_loggedin) {
    ...
    let now = new Date(d.getTime() - d.getTimezoneOffset() * 60000)
      ...
      .slice(0, -1)
      .split(".")[0];
    connection.query("UPDATE accounts SET last_seen = ? WHERE id = ?", [
      now,
...
const loginAttempts = (ip, update = true, callback) => {
  ...
  console.log(ip, "ipaddress inside loginAttempts");
  let d = new Date();
  let now = new Date(d.getTime() - d.getTimezoneOffset() * 60000)
    .toISOString()
    .slice(0, -1)
    ...
    (error, results) => {
      ...
      if (results.length > 0) {
        ...
        if (d.getTime() > expire.getTime()) {
          // Delete attempts
          connection.query("DELETE FROM login_attempts WHERE id_address = ?", [
            ip,
          ]);
        } else {
...
const getSettings = (callback) => {
  connection.query(
    ...
    (error, settings, fields) => {
      settings2 = {};
      for (let setting in settings) {
        settings2[settings[setting]["setting_key"]] = {
          key: settings[setting]["setting_key"],
          value: settings[setting]["setting_value"],
          category: settings[setting]["category"],
        };
      }
      callback(settings2);
...
const timeElapsedString = (date) => {
  let seconds = Math.floor(
    (new Date() - new Date(String(date).replace(/-/g, "/"))) / 1000
  );
